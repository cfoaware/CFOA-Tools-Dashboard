...
function LoginScreen({ onLogin, dark, toggleDark }) {
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background: dark ? "#0f1117" : "linear-gradient(145deg,#fff8f5 0%,#fff3e8 50%,#fdf0e0 100%)", padding:24, transition:"background 0.3s" }}>
      ...
      <div style={{ position:"fixed", top:20, right:20 }}>
        <button className="toggle-btn" onClick={toggleDark}>{dark ? "☀️" : "🌙"} {dark ? "Light" : "Dark"}</button>
      </div>
      <div className={`fade-up${shaking ? " shake" : ""}`} style={{ background: dark ? "#1a1d27" : "#fff", border:`1px solid ${dark ? "#2a2d3a" : "#e8eaed"}`, borderRadius:20, padding:"52px 44px", width:"100%", maxWidth:400, textAlign:"center", boxShadow:`0 8px 40px rgba(0,0,0,${dark ? "0.4" : "0.08"})`, transition:"background 0.3s" }}>
        ...
        <button onClick={attempt} style={{ width:"100%", padding:"14px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#e8450a 0%,#f7a01a 100%)", color:"#fff", fontSize:15, fontWeight:700, fontFamily:"'Nunito Sans',sans-serif", cursor:"pointer", boxShadow:"0 4px 18px rgba(232,69,10,0.28)" }} onMouseEnter={e=>e.currentTarget.style.opacity="0.88"} onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
          Access Library →
        </button>
    ...
    <div style={{ minHeight:"100vh", background: dark ? "#0f1117" : "#f4f5f7", transition:"background 0.3s" }}>
      <div className="header-bar" style={{ padding:"0 40px", position:"sticky", top:0, zIndex:10, display:"flex", alignItems:"center", justifyContent:"space-between", height:68 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <button className="toggle-btn" onClick={toggleDark}>{dark ? "☀️" : "🌙"} {dark ? "Light mode" : "Dark mode"}</button>
...
export default function App() {
  ...
  return (
    <>
      ...
      {authed ? <Portal dark={dark} toggleDark={toggleDark} /> : <LoginScreen onLogin={handleLogin} dark={dark} toggleDark={toggleDark} />}
    </>
  );
}
